...
//    @Query("select c from Customer c where c.loyalty.id = :loyaltyId")
    ...
    @Query("SELECT NEW meu.booking_rebuild_ver2.response.Passanger.CustomerResponse(c.id, c.name, c.phone, " +
            "NEW meu.booking_rebuild_ver2.model.Admin.DTO.LoyaltyDto(c.loyalty), c.status.status, c.numberOfTrips) " +
